...
void LCD_Fill(u16 xsta, u16 ysta, u16 xend, u16 yend, u16 color) {
    ...
    while (t) {
        ...
        // 	}
        // }
        wk_dma1_channel3_init(FALSE);
        wk_dma_channel_config(USA_DMA_CHANNEL, (uint32_t)&SPI1->dt, (u32)color1, num1);
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void LCD_ShowPicture(u16 x, u16 y, u16 length, u16 width, const uint16_t pic[]) {
    u8 t = 1;
    u32 num = length * width, num1;
    ...
    while (t) {
        ...
        wk_dma_channel_config(USA_DMA_CHANNEL, (uint32_t)&SPI1->dt, (u32)pic, num1);
        dma_channel_enable(USA_DMA_CHANNEL, TRUE);
        // MYDMA_Enable(DMA1_Channel3);
        while (1) {
            if (dma_flag_get(DMA1_FDT3_FLAG) != RESET)  // 等待通道4传输完成
